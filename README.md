...
| **[`kafka-consumer.md`](docs/kafka-consumer.md)** | Processor consumer: per-partition batching, commits, DLQ, scaling, shutdown drain |
...
| `KAFKA_BATCH_SIZE`, `KAFKA_BATCH_FLUSH_INTERVAL_SEC` | Processor: flush Postgres batch when buffer reaches size or interval (per partition) |
| `KAFKA_SHUTDOWN_DRAIN_TIMEOUT_SEC` | Max wait for partition workers during graceful shutdown |
