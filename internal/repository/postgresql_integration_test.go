...
func TestPostgresRepo_IntegrationFlow(t *testing.T) {
	...
	t.Run("ok/save_bulk_inserts_multiple", func(t *testing.T) {
		u := int64(77)
		ts := time.Now().UTC().Truncate(time.Microsecond)
		txs := []domain.Transaction{
			{UserID: u, Type: domain.TransactionTypeBet, Amount: 10, Timestamp: ts},
			{UserID: u, Type: domain.TransactionTypeWin, Amount: 20, Timestamp: ts.Add(time.Minute)},
		}
		if err := repo.SaveBulk(ctx, txs); err != nil {
			t.Fatalf("SaveBulk() error = %v", err)
		}
		res, err := repo.Get(ctx, u, nil)
		if err != nil {
			t.Fatalf("Get() error = %v", err)
		}
		if len(res) != 2 {
			t.Fatalf("len(Get) = %d, want 2", len(res))
		}
	})
