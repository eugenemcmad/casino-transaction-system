...
func (s *transactionService) RegisterTransactions(ctx context.Context, txs []domain.Transaction) error {
	slog.Debug("Registering transactions in bulk", "count", len(txs))
	return s.repo.SaveBulk(ctx, txs)
}
