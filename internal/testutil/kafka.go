...
// It binds a free host port and maps it into the container so advertised listeners match the client address.
func SetupKafka(t *testing.T) (string, func()) {
	t.Helper()
	defer func() {
		if r := recover(); r != nil {
			onTestcontainerSetupError(t, fmt.Errorf("kafka setup panic: %v", r))
		}
	}()
	...
	if err != nil {
		onTestcontainerSetupError(t, err)
	}
...
func CreateTopicAndWait(t *testing.T, broker, topic string) {
	...
	defer func() {
		if closeErr := conn.Close(); closeErr != nil {
			t.Errorf("failed to close Kafka connection: %v", closeErr)
		}
	}()
